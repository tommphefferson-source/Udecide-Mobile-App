...
**Google Civic is NOT a fallback for reps.** Google Civic Information API's `representatives` endpoint (`civicinfo/v2/representatives`) was deprecated and shut down — with a *valid* key it returns `HTTP 404 "Method not found"` (an invalid key returns 400 first, which can look like the endpoint is alive — it isn't). `divisionsByAddress` only returns OCD division IDs, not official names/contacts. So there is no working Google Civic source for county/city officials; don't wire it up expecting reps data. Cicero is the source; where Cicero lacks local coverage (common in smaller towns; county data sparse), honestly show "not available."
