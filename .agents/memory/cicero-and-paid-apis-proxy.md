...
**Google Civic: only `representatives` is dead; `elections`/`voterinfo` still work.** The `civicinfo/v2/representatives` endpoint was shut down — with a *valid* key it returns `HTTP 404 "Method not found"` (an invalid key returns 400 first, which can look alive — it isn't). `divisionsByAddress` only returns OCD division IDs, not official names/contacts. So there is NO Google Civic source for county/city officials; Cicero is the reps source, and where Cicero lacks local coverage (common in smaller towns; county data sparse) honestly show "not available." BUT `civicinfo/v2/elections` (200) and `civicinfo/v2/voterinfo?address=&electionId=` ARE live and used by the api-server elections endpoints — `voterinfo` 400s ("Election unknown") without a valid current `electionId`; map upstream 400s to client 400 and other failures to 502. The key secret is `CIVIC_API_KEY` (api-server config also reads `GOOGLE_CIVIC_API_KEY` first, then falls back).
